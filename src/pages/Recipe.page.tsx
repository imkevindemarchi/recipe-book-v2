...
const Recipe: FC = () => {
  ...
  const info: ReactNode = (
    <div className="flex flex-col">
      <div
        className={`transition-all duration-300 absolute mt-[-2vh] rounded-3xl w-full h-10 ${
          isDarkMode ? "bg-black" : "bg-white"
        }`}
      />
      <div
        className={`transition-all duration-300 mt-5 p-10 rounded-3xl w-full flex flex-col gap-5 mobile:p-5 ${
          isDarkMode ? "bg-black" : "bg-white"
        }`}
      >
        ...
        <Stepper
          isDarkMode={isDarkMode}
          steps={recipe?.procedure as string[]}
        />
      </div>
    </div>
  );
