...
const AdminIngredients: FC = () => {
  ...
  const pageTitle: string = t("ingredients");
  const selectedIngredientId: string = selectedIngredient?.id as string;
  ...
  async function onDelete(): Promise<void> {
    ...
    await Promise.resolve(INGREDIENT_API.delete(selectedIngredientId)).then(
      async (response: THTTPResponse) => {
        if (response && response.hasSuccess) {
          openPopup(t("ingredientDeleted"), "success");
          await getData();
  ...
  const deleteModalComponent: ReactNode = (
    ...
    >
      ...
      >
        {t("confirmToDelete", { name: selectedIngredient?.label })}
      </span>
    </Modal>
  );
